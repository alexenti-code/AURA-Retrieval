...
The figures above characterize the disclosed test configuration only.

- **Dataset:** dialogues generated by `synthetic.py` (synthetic dialogue-override
  scenarios). No external dataset is used.
- **Metric:** recall@0.8 = fraction of Q/A probes whose retrieved-to-original
  cosine similarity is at least 0.8 (`retention_threshold` in `core.py`),
  computed over 300 probes after a 1000-turn dialogue run that wrote 2041 facts.
- **Seeds:** experiment scripts use a fixed seed (`torch.manual_seed(42)`);
  the rank-1 write path itself is deterministic — no gradient steps, no sampling.
- **Hardware:** one consumer Apple Silicon laptop, CPU only. The substrate is
  resident in RAM during hot-path measurements; cold-write figures include
  loading the region from disk.
- **Baselines:** no end-to-end baseline systems were run in this release.
  The economics table above is a FLOP cost model, not a measured comparison.
- **Scope:** single-run measurements of the disclosed benchmark. They are not
  a claim of general model performance. Capacity figures assume no
  consolidation (consolidation mechanisms are not disclosed).
